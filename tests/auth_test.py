...
from LMOktaLogsForwarder import log_ingester as li
from LMOktaLogsForwarder import msgspec_okta_event as moe
...
def test_no_auth_specified(mocker):
    with pytest.raises(ValueError):
        ...
        mocker.patch("LMOktaLogsForwarder.storage_account.getOktaUrl", return_value=None)
        log_ingester = li.LogIngester()
        

def test_partial_lmv1_specified_with_no_bearer(partial_lmv1_defined_with_no_bearer):
    with pytest.raises(ValueError):
        with mock.patch("LMOktaLogsForwarder.storage_account.getOktaUrl") as mck:
            mck.side_effect = partial_lmv1_defined_with_no_bearer
            log_ingester = li.LogIngester()
            


def test_only_lmv1_specified_with_no_bearer(mocker,getOktaUrl_only_LMV1_defined, mock_log_ingest_post):
    with mock.patch("LMOktaLogsForwarder.storage_account.getOktaUrl") as mck:
        mck.side_effect = getOktaUrl_only_LMV1_defined
        log_ingester = li.LogIngester()
...
def test_all_auth_specified(mocker,getOktaUrl_all_defined, mock_log_ingest_post):
    with mock.patch("LMOktaLogsForwarder.storage_account.getOktaUrl") as mck:
        mck.side_effect = getOktaUrl_all_defined
        log_ingester = li.LogIngester()
        mocker.patch("time.time", return_value=123456789)
        raw_resp_from_okta = moe.loads(open("tests/data/resp.json").read())
        log_ingester.ingest_to_lm_logs(raw_resp_from_okta)
...
@pytest.fixture(name="getOktaUrl_only_LMV1_defined", scope="session")
def fixture_mock_only_LMV1_defined():
    def _getOktaUrl2(key):
        returns = {
            "id" : "some-string",
            "key" : "some-string",
            "token" : None
...
@pytest.fixture(name="getOktaUrl_all_defined",scope="session")
def fixture_mock_all_auth_defined():
    def _getOktaUrl3(key):
        returns = {
            "id" : "some-string",
            "key" : "some-string",
            "token" : "some-string"
