...
def mock_log_ingester(mocker):
    mocker.patch("LMOktaLogsForwarder.storage_account.getOktaUrl", return_value="some-string")
    mocker.patch("LMOktaLogsForwarder.log_ingester.LogIngester.get_company_name", return_value="mycompany")
    mocker.patch("LMOktaLogsForwarder.log_ingester.LogIngester.ingest_to_lm_logs")
...
@pytest.fixture()
def mock_storage_account_read_link(mocker):
    mocker.patch("LMOktaLogsForwarder.storage_account.getOktaUrl", return_value="some-string")
    mocker.patch("LMOktaLogsForwarder.storage_account.getOktaUrl", return_value=json.dumps({'okta_next_link':'https://somedomain.com/some_link_read_from_s3', 'next_link_retries' : 1}))
